...
export class PrepareController {
  ...
  async fillAttendees() {
    ...
    for (let item of attendees) {
      ...
      const event_dates = await this.eventsService.getEventDatesByEvent(
        item.event_id,
      );
      /*const currentEvent = await this.eventsService.getCurrentEvent(
        event_dates,
      );
  ...
  @ApiExcludeEndpoint()
  async createData() {
    await this.prepareService.deleteData();
    await this.prepareService.createEvents();
    await this.prepareService.createEventDate();
    await this.prepareService.createAttendees();
    await this.prepareService.signAttendance();
  }
}
